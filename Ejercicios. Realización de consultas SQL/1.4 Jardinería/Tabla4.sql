...
SELECT nombre + ' ' + apellido1 + ' ' apellido2, puesto FROM empleado WHERE puesto != 'representantes de ventas';
...
SELECT DISTINCT codigo_cliente, YEAR(fecha_pago) FROM pago WHERE YEAR(fecha_pago) = 2008;

--Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos que no han sido entregados a tiempo.
SELECT codigo_pedido, codigo_cliente, fecha_esperada, estado, fecha_entrega FROM pedido WHERE estado = 'Pendiente';

--Devuelve un listado con el código de pedido, código de cliente, fecha esperada y fecha de entrega de los pedidos cuya fecha de entrega ha sido al menos dos días antes de la fecha esperada.
SELECT codigo_pedido, codigo_cliente, estado, fecha_entrega, fecha_esperada FROM pedido WHERE fecha_entrega IS NOT NULL AND fecha_entrega < DATEADD(day, -2, fecha_esperada);

--Devuelve un listado de todos los pedidos que fueron rechazados en 2009.
SELECT codigo_pedido, codigo_cliente, estado, fecha_esperada, fecha_entrega FROM pedido WHERE estado = 'Rechazado' AND YEAR(fecha_entrega) = 2009;

--Devuelve un listado de todos los pedidos que han sido entregados en el mes de enero de cualquier año.
SELECT codigo_pedido, codigo_cliente, estado, fecha_esperada, fecha_entrega FROM pedido WHERE estado = 'Entregado' AND MONTH(fecha_entrega) = 1;

--Devuelve un listado con todos los pagos que se realizaron en el año 2008 mediante Paypal. Ordene el resultado de mayor a menor.
SELECT id_transaccion, codigo_cliente, forma_pago, fecha_pago, total FROM pago WHERE forma_pago = 'Paypal' AND YEAR(fecha_pago) = 2008;

--Devuelve un listado con todas las formas de pago que aparecen en la tabla pago. Tenga en cuenta que no deben aparecer formas de pago repetidas.
SELECT DISTINCT forma_pago FROM pago;

--Devuelve un listado con todos los productos que pertenecen a la gama Ornamentales y que tienen más de 100 unidades en stock. El listado deberá estar ordenado por su precio de venta, mostrando en primer lugar los de mayor precio.
SELECT nombre, precio_venta, cantidad_en_stock FROM producto WHERE gama = 'Ornamentales' AND cantidad_en_stock > 100;

--Devuelve un listado con todos los clientes que sean de la ciudad de Madrid y cuyo representante de ventas tenga el código de empleado 11 o 30.
SELECT nombre_cliente, codigo_empleado_rep_ventas, empleado.nombre FROM cliente INNER JOIN empleado ON codigo_empleado_rep_ventas = codigo_empleado WHERE ciudad = 'Madrid' AND codigo_empleado IN(11,30);

--1.4.5 Consultas multitabla (Composición interna)
--Obtén un listado con el nombre de cada cliente y el nombre y apellido de su representante de ventas.
SELECT nombre_cliente, nombre, apellido1, apellido2 FROM cliente INNER JOIN empleado ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas;

--Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT DISTINCT nombre_cliente, nombre, apellido1, apellido2 FROM cliente INNER JOIN empleado ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado INNER JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente;

--Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
SELECT nombre_cliente, nombre, apellido1, apellido2 FROM cliente INNER JOIN empleado ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado WHERE codigo_cliente NOT IN(SELECT codigo_cliente FROM pago);

--Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT cliente.nombre_cliente, empleado.nombre, empleado.apellido1, empleado.apellido2, oficina.ciudad FROM cliente INNER JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente INNER JOIN empleado ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado INNER JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina;

--Devuelve el nombre de los clientes que no hayan hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT cliente.nombre_cliente, empleado.nombre, empleado.apellido1, empleado.apellido2, oficina.ciudad FROM oficina INNER JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina INNER JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas WHERE cliente.codigo_cliente NOT IN(SELECT pago.codigo_cliente FROM pago);

--Lista la dirección de las oficinas que tengan clientes en Fuenlabrada.
SELECT oficina.linea_direccion1, oficina.linea_direccion2, oficina.ciudad FROM oficina INNER JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina INNER JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas WHERE cliente.ciudad = 'Fuenlabrada';

--Devuelve el nombre de los clientes y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
SELECT cliente.nombre_cliente, empleado.nombre, empleado.apellido1, empleado.apellido2, oficina.ciudad FROM oficina INNER JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina INNER JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas;

--Devuelve un listado con el nombre de los empleados junto con el nombre de sus jefes.
SELECT empleado.nombre, empleado.apellido1, empleado.apellido2, jefe.nombre, jefe.apellido1, jefe.apellido2 FROM empleado INNER JOIN empleado jefe ON empleado.codigo_jefe = jefe.codigo_empleado;

--Devuelve un listado que muestre el nombre de cada empleados, el nombre de su jefe y el nombre del jefe de sus jefe.
SELECT empleado.nombre AS empleado, jefe.nombre AS jefe, Boss.nombre AS Boss FROM empleado LEFT JOIN empleado jefe ON empleado.codigo_jefe = jefe.codigo_empleado LEFT JOIN empleado Boss ON jefe.codigo_jefe = Boss.codigo_empleado;

--Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un pedido.
SELECT cliente.nombre_cliente, pedido.fecha_entrega, pedido.fecha_esperada FROM cliente INNER JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente WHERE pedido.fecha_entrega > pedido.fecha_esperada;

--Devuelve un listado de las diferentes gamas de producto que ha comprado cada cliente.
SELECT DISTINCT(gama), nombre_cliente FROM cliente INNER JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente INNER JOIN detalle_pedido ON pedido.codigo_pedido = detalle_pedido.codigo_pedido INNER JOIN producto ON detalle_pedido.codigo_producto = producto.codigo_producto;
...
SELECT DISTINCT cliente.nombre_cliente + ' ' + cliente.apellido_contacto, pago.id_transaccion, pedido.codigo_pedido, pedido.fecha_pedido, pedido.estado FROM cliente LEFT JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente LEFT JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente WHERE pago.codigo_cliente iS NULL AND pedido.codigo_cliente IS NULL;

--Devuelve un listado que muestre solamente los empleados que no tienen una oficina asociada.
SELECT empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, oficina.linea_direccion1 FROM empleado LEFT JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina WHERE empleado.codigo_oficina IS NULL;

--Devuelve un listado que muestre solamente los empleados que no tienen un cliente asociado.
SELECT empleado.puesto, empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, cliente.nombre_cliente + ' ' + cliente.apellido_contacto FROM empleado LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas WHERE cliente.codigo_empleado_rep_ventas IS NULL;
...
SELECT empleado.puesto, empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, cliente.nombre_cliente + ' ' + cliente.apellido_contacto, oficina.linea_direccion1 + ' ' + oficina.linea_direccion2 FROM empleado LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas LEFT JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina WHERE cliente.codigo_empleado_rep_ventas IS NULL;


--Devuelve un listado que muestre los empleados que no tienen una oficina asociada y los que no tienen un cliente asociado.
SELECT empleado.puesto, empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, cliente.nombre_cliente + ' ' + cliente.apellido_contacto FROM empleado LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas WHERE cliente.codigo_empleado_rep_ventas IS NULL OR empleado.codigo_oficina IS NULL;

--Devuelve un listado de los productos que nunca han aparecido en un pedido.
SELECT DISTINCT producto.nombre, producto.codigo_producto, detalle_pedido.codigo_producto FROM producto LEFT JOIN detalle_pedido ON producto.codigo_producto = detalle_pedido.codigo_producto WHERE detalle_pedido.codigo_producto IS NULL;

--Devuelve un listado de los productos que nunca han aparecido en un pedido. El resultado debe mostrar el nombre, la descripción y la imagen del producto.
SELECT producto.nombre, producto.descripcion, gama_producto.imagen FROM producto LEFT JOIN detalle_pedido ON producto.codigo_producto = detalle_pedido.codigo_producto LEFT JOIN gama_producto ON producto.gama = gama_producto.gama WHERE detalle_pedido.codigo_producto IS NULL;

--Devuelve las oficinas donde no trabajan ninguno de los empleados que hayan sido los representantes de ventas de algún cliente que haya realizado la compra de algún producto de la gama Frutales.
SELECT oficina.* FROM oficina JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente JOIN detalle_pedido ON pedido.codigo_pedido = detalle_pedido.codigo_pedido JOIN producto ON detalle_pedido.codigo_producto = producto.codigo_producto JOIN gama_producto ON producto.gama = gama_producto.gama WHERE gama_producto.gama != 'Frutales' AND (cliente.codigo_empleado_rep_ventas <> ALL(SELECT codigo_empleado FROM empleado JOIN oficina ON oficina.codigo_oficina = empleado.codigo_oficina));

--Devuelve un listado con los clientes que han realizado algún pedido pero no han realizado ningún pago.
SELECT cliente.nombre_cliente + ' ' + cliente.apellido_contacto, pedido.codigo_pedido, pago.id_transaccion, pedido.fecha_pedido, pedido.estado FROM cliente LEFT JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente LEFT JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente WHERE pago.codigo_cliente IS NULL AND pedido.codigo_cliente IS NOT NULL;

--Devuelve un listado con los datos de los empleados que no tienen clientes asociados y el nombre de su jefe asociado.
SELECT empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, empleado.puesto, cliente.nombre_cliente + ' ' + cliente.apellido_contacto, empleado.codigo_empleado,empleado.codigo_oficina, Boss.nombre + ' ' + Boss.apellido1 + ' ' + Boss.apellido2 FROM empleado LEFT JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas LEFT JOIN empleado Boss ON empleado.codigo_jefe = Boss.codigo_empleado WHERE cliente.codigo_empleado_rep_ventas IS NULL;
...
SELECT estado, COUNT(*) AS Total FROM pedido GROUP BY estado ORDER BY Total DESC;
...
SELECT empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, COUNT(*) FROM empleado, cliente WHERE empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas GROUP BY empleado.nombre, empleado.apellido1, empleado.apellido2;
...
SELECT cliente.nombre_contacto + ' ' + cliente.apellido_contacto, MIN(pago.fecha_pago), MAX(pago.fecha_pago) FROM cliente, pago WHERE cliente.codigo_cliente = pago.codigo_cliente GROUP BY cliente.nombre_contacto, cliente.apellido_contacto;

--Calcula el número de productos diferentes que hay en cada uno de los pedidos.
SELECT COUNT(DISTINCT detalle_pedido.codigo_producto) FROM pedido, detalle_pedido WHERE pedido.codigo_pedido = detalle_pedido.codigo_pedido;

--Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de los pedidos.
SELECT SUM(detalle_pedido.cantidad) FROM pedido, detalle_pedido WHERE pedido.codigo_pedido = detalle_pedido.codigo_pedido;
...
SELECT TOP(20) producto.nombre, SUM(detalle_pedido.cantidad) AS Total FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto GROUP BY producto.nombre ORDER BY Total DESC;

--La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, el IVA y el total facturado. La base imponible se calcula sumando el coste del producto por el número de unidades vendidas de la tabla detalle_pedido. El IVA es el 21 % de la base imponible, y el total la suma de los dos campos anteriores.
SELECT SUM(detalle_pedido.cantidad * producto.precio_venta), SUM((detalle_pedido.cantidad * producto.precio_venta) * 0.21), SUM(detalle_pedido.cantidad * producto.precio_venta * 1.21) FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto;

--La misma información que en la pregunta anterior, pero agrupada por código de producto.
SELECT SUM(detalle_pedido.cantidad * producto.precio_venta), SUM((detalle_pedido.cantidad * producto.precio_venta) * 0.21), SUM(detalle_pedido.cantidad * producto.precio_venta * 1.21) FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto GROUP BY producto.codigo_producto, producto.nombre;
...
SELECT SUM(detalle_pedido.cantidad * producto.precio_venta), SUM((detalle_pedido.cantidad * producto.precio_venta) * 0.21), SUM(detalle_pedido.cantidad * producto.precio_venta * 1.21) FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto AND producto.codigo_producto LIKE 'OR%' GROUP BY producto.codigo_producto, producto.nombre;
...
SELECT SUM(detalle_pedido.cantidad * producto.precio_venta), SUM((detalle_pedido.cantidad * producto.precio_venta) * 0.21), SUM(detalle_pedido.cantidad * producto.precio_venta * 1.21) FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto AND (detalle_pedido.cantidad * producto.precio_venta * 1.21) > 3000 GROUP BY producto.nombre;

--Muestre la suma total de todos los pagos que se realizaron para cada uno de los años que aparecen en la tabla pagos.
SELECT SUM(pago.total), YEAR(pago.fecha_pago) FROM pago GROUP BY YEAR(pago.fecha_pago);
...
SELECT TOP(1) nombre, SUM(cantidad) FROM producto, detalle_pedido WHERE producto.codigo_producto = detalle_pedido.codigo_producto GROUP BY producto.codigo_producto, nombre;

--Los clientes cuyo límite de crédito sea mayor que los pagos que haya realizado. (Sin utilizar INNER JOIN).
SELECT nombre_cliente, limite_credito FROM cliente WHERE limite_credito > (SELECT SUM(total) FROM pago WHERE codigo_cliente = cliente.codigo_cliente);

--Devuelve el producto que más unidades tiene en stock.
SELECT nombre, producto.cantidad_en_stock FROM producto WHERE producto.cantidad_en_stock = (SELECT MAX(producto.cantidad_en_stock) FROM producto);

--Devuelve el producto que menos unidades tiene en stock.
SELECT nombre, producto.cantidad_en_stock FROM producto WHERE producto.cantidad_en_stock = (SELECT MIN(producto.cantidad_en_stock) FROM producto);

--Devuelve el nombre, los apellidos y el email de los empleados que están a cargo de Alberto Soria.
SELECT nombre + ' ' + apellido1, email, codigo_jefe FROM empleado WHERE codigo_jefe = (SELECT codigo_empleado FROM empleado WHERE nombre = 'Alberto' AND apellido1 = 'Soria');
...
--Devuelve el producto que menos unidades tiene en stock.
SELECT nombre, producto.cantidad_en_stock FROM producto WHERE producto.cantidad_en_stock <= ALL(SELECT producto.cantidad_en_stock FROM producto);

--1.4.8.3 Subconsultas con IN y NOT IN
--Devuelve el nombre, apellido1 y cargo de los empleados que no representen a ningún cliente.
SELECT nombre, apellido1, puesto, email, codigo_empleado FROM empleado WHERE codigo_empleado NOT IN (SELECT codigo_empleado_rep_ventas FROM cliente WHERE codigo_empleado_rep_ventas = codigo_empleado);

--Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.
SELECT nombre_cliente FROM cliente WHERE codigo_cliente NOT IN (SELECT codigo_cliente FROM pago WHERE codigo_cliente = codigo_cliente);

--Devuelve un listado que muestre solamente los clientes que sí han realizado algún pago.
SELECT nombre_cliente FROM cliente WHERE codigo_cliente IN (SELECT codigo_cliente FROM pago WHERE codigo_cliente = codigo_cliente);

--Devuelve un listado de los productos que nunca han aparecido en un pedido.
SELECT nombre FROM producto WHERE codigo_producto NOT IN (SELECT codigo_producto FROM detalle_pedido WHERE codigo_producto = codigo_producto);

--Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que no sean representante de ventas de ningún cliente.
SELECT nombre, apellido1, apellido2, telefono FROM empleado INNER JOIN oficina on empleado.codigo_oficina = oficina.codigo_oficina WHERE codigo_empleado NOT IN (SELECT codigo_empleado_rep_ventas FROM cliente WHERE codigo_empleado_rep_ventas = codigo_empleado);

--Devuelve las oficinas donde no trabajan ninguno de los empleados que hayan sido los representantes de ventas de algún cliente que haya realizado la compra de algún producto de la gama Frutales.
SELECT oficina.* FROM oficina JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina JOIN cliente ON empleado.codigo_empleado = cliente.codigo_empleado_rep_ventas JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente JOIN detalle_pedido ON pedido.codigo_pedido = detalle_pedido.codigo_pedido JOIN producto ON detalle_pedido.codigo_pedido = producto.codigo_producto JOIN gama_producto ON producto.gama = gama_producto.gama WHERE gama_producto.gama != 'Frutales' AND (cliente.codigo_empleado_rep_ventas NOT IN(SELECT codigo_empleado_rep_ventas FROM cliente));

--Devuelve un listado con los clientes que han realizado algún pedido pero no han realizado ningún pago.
SELECT nombre_cliente AS nombre , estado from cliente INNER JOIN pedido ON cliente.codigo_cliente=pedido.codigo_cliente INNER JOIN pago ON pedido.codigo_cliente=pago.codigo_cliente WHERE cliente.codigo_cliente IN (SELECT codigo_cliente FROM pago WHERE codigo_cliente = pedido.codigo_cliente);

--1.4.8.4 Subconsultas con EXISTS y NOT EXISTS
--Devuelve un listado que muestre solamente los clientes que no han realizado ningún pago.
SELECT nombre_cliente AS Nombre FROM cliente AS c LEFT JOIN pago AS p ON c.codigo_cliente=p.codigo_cliente WHERE NOT EXISTS (SELECT p.codigo_cliente FROM pago WHERE codigo_cliente = c.codigo_cliente);

--Devuelve un listado que muestre solamente los clientes que sí han realizado algún pago.
SELECT nombre_cliente AS Nombre FROM cliente AS c LEFT JOIN pago AS p ON c.codigo_cliente=p.codigo_cliente WHERE EXISTS (SELECT p.codigo_cliente FROM pago WHERE codigo_cliente = c.codigo_cliente);

--Devuelve un listado de los productos que nunca han aparecido en un pedido.
SELECT nombre AS Nombre_Producto FROM producto AS p LEFT JOIN detalle_pedido AS dp ON p.codigo_producto=dp.codigo_producto WHERE NOT EXISTS (SELECT nombre FROM producto WHERE p.codigo_producto = dp.codigo_producto);

--Devuelve un listado de los productos que han aparecido en un pedido alguna vez.
SELECT nombre AS Nombre_Producto FROM producto AS p LEFT JOIN detalle_pedido AS dp ON p.codigo_producto=dp.codigo_producto WHERE EXISTS (SELECT nombre FROM producto WHERE p.codigo_producto = dp.codigo_producto);
...
SELECT nombre_cliente, COUNT(pedido.codigo_pedido) FROM cliente LEFT JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente GROUP BY nombre_cliente;
...
SELECT DISTINCT nombre_cliente, YEAR(fecha_pedido) FROM cliente LEFT JOIN pedido ON cliente.codigo_cliente = pedido.codigo_cliente WHERE YEAR(fecha_pedido) = 2008;
...
SELECT nombre_cliente, pago.id_transaccion, empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, oficina.telefono FROM cliente LEFT JOIN empleado ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado LEFT JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina LEFT JOIN pago ON cliente.codigo_cliente = pago.codigo_cliente WHERE pago.codigo_cliente IS NULL;
...
SELECT nombre_cliente, empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, oficina.ciudad FROM cliente LEFT JOIN empleado ON cliente.codigo_empleado_rep_ventas = empleado.codigo_empleado LEFT JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina ORDER BY nombre_cliente; 

--Devuelve el nombre, apellidos, puesto y teléfono de la oficina de aquellos empleados que no sean representante de ventas de ningún cliente.
SELECT empleado.nombre + ' ' + empleado.apellido1 + ' ' + empleado.apellido2, empleado.puesto, oficina.telefono FROM empleado LEFT JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina WHERE empleado.codigo_empleado NOT IN(SELECT codigo_empleado_rep_ventas FROM cliente);

--Devuelve un listado indicando todas las ciudades donde hay oficinas y el número de empleados que tiene.
SELECT oficina.ciudad, COUNT(empleado.codigo_empleado) FROM oficina LEFT JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina GROUP BY oficina.ciudad;
